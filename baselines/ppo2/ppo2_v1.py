...
class Model(object):
    def __init__(self, *, policy, ob_space, obs_dict, ac_space, nbatch_act, nbatch_train,
                nsteps, ent_coef, vf_coef, max_grad_norm):
        sess = tf.get_default_session()

        act_model = policy(sess, ob_space, obs_dict,ac_space, nbatch_act, 1, reuse=False)
        train_model = policy(sess, ob_space,obs_dict, ac_space, nbatch_train, nsteps, reuse=True)
...
def learn(*, policy, env, nsteps, total_timesteps, ent_coef, lr,obs_dict,
    ...
    make_model = lambda : Model(policy=policy, ob_space=ob_space, obs_dict=obs_dict, ac_space=ac_space, nbatch_act=nenvs, nbatch_train=nbatch_train,
